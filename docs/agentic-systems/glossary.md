...
* **Agent Runtime** — The deterministic execution environment that wraps every agent process. One runtime per agent.
  ...
  tools from the capability set including runtime protocol tools, gates resolution against pending continuations, manages
  continuation deadlines, handles cancellation fan-out to sub-agents, validates LLM protocol actions against topic state,
...
* **LLM Gateway** — An external process (not a mailbox actor) that agents connect to via gRPC streaming for inference.
  Manages provider connections (Ollama, OpenAI-compatible, etc.), auth rotation, failover, load balancing, and
  operational
