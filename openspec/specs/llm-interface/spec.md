...
Defines the unified interface for all LLM providers, decoupling consumers from
provider-specific types and enabling interchangeable backends.
...
- **THEN** the request is cancelled and returns context.Canceled or
  context.DeadlineExceeded (not wrapped — chain treats this as termination, not model
  failure)
